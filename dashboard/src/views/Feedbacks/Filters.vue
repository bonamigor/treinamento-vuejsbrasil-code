...
const applyFiltersStructures = (summary) => {
...
export default {
  async setup (props, { emit }) {
    ...
    try {
      ...
      state.filters = applyFiltersStructures(data)
    } catch (error) {
      state.hasError = !!error
      state.filters = applyFiltersStructures({ all: 0, issue: 0, idea: 0, other: 0 })
    }
    ...
    const handleSelect = ({ type }) => {
      ...
      state.filters = state.filter.map((filter) => {
        if (filter.type === type) {
